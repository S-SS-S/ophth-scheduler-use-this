...
test('language, theme, and font controls remain interactive', async ({ page }) => {
  ...
  await expect(page.locator('#title')).toContainText('Ophthalmology');
  await expect(page.locator('#actionsTitle')).toContainText('Result tools');
...
test('retinal laser produces a yearly follow-up result', async ({ page }) => {
  ...
  await expect(page.locator('#details')).toContainText('إجراء ليزر للشبكية');
...
test('result tools are localized', async ({ page }) => {
  ...
  await expect(page.locator('#disclaimer')).toContainText('educational tool');
});